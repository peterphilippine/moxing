#%%writefile /usr/local/lib/python3.11/dist-packages/triton/runtime/autotuner.py
...
class Autotuner(KernelInterface):
    ...
    def run(self, *args, **kwargs):
            ...
            if key not in self.cache:
                ...
                def benchmark():
                    ...
                    if self.nargs:
                        full_nargs.update(self.nargs)
                    if kwargs:
                        full_nargs.update(kwargs)
                    if self.cache[key].all_kwargs():
                        full_nargs.update(self.cache[key].all_kwargs())
        ...
        if config.pre_hook is not None:
            ...
            if kwargs:
                full_nargs.update(kwargs)
            if config.all_kwargs():
                full_nargs.update(config.all_kwargs())
            config.pre_hook(full_nargs)
        ret = self.fn.run(
            *args,
            **kwargs,
